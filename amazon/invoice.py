...
def print_pdf(order, browser):
    ...
    try:
        ...
        pdfkit.from_string(html.prettify(), filename, options=options)
        # with open("{}.html".format(order_id), 'wb') as fil:
        #     fil.write(html.prettify())
        order["pdf"] = filename
    except Exception as ex:
        err_msg = str(ex)
        # this is to handle an open issue in wkhtmltopdf
        if "ProtocolInvalidOperationError" in err_msg:
            return True
        raise ex
...
def generate(auth, timeframe, clean):
    """
    main controller
    """
    browser = get_browser()
...
def clean_up(tmp_dir="./static/assets/db/"):
    """
    Cleans up any auxiliary file generated by invoice
    """
    import shutil
...
@click.option('--clean', '-c', is_flag=True, help='Clean up auxiliary files after generating pdf')
def cli(username, password, start, end, clean):
    ...
    click.echo(str(generate(auth, timeframe, clean)))
    if clean:
        if clean_up():
...
class TestAmazon(unittest.TestCase):
    ...
    def test_clean_up(self):
        import string
        import random
        name = ''.join(random.choice(string.letters) for _ in range(9))
        tmp_dir = "/tmp/{}".format(name)
        while os.path.exists(tmp_dir):
            name = ''.join(random.choice(string.letters) for _ in range(9))
            tmp_dir = "/tmp/{}".format(name)
        self.assertEqual(clean_up(tmp_dir), not os.path.exists(tmp_dir))
        os.makedirs(tmp_dir)
        self.assertEqual(clean_up(tmp_dir), not os.path.exists(tmp_dir))
        self.assertEqual(clean_up(tmp_dir), not os.path.exists(tmp_dir))
